...
contract miningAuthmen is Initializable, IERC1155ReceiverUpgradeable {
    ...
    mapping(address => mapping(uint256 => StakeInfo)) public accountsNFTInfo;
    ...
    function nftMiningNumbersPerSecond(uint256 nftLevel)
        internal
        pure
        returns (uint256)
    {
    ...
    function getAccountNFTEaring(address account, uint256 nftLevel)
        internal
        view
        returns (uint256)
    {
        ...
        uint256 timeLong =
            now.sub(accountsNFTInfo[account][nftLevel].stakeTime);
        uint256 amount = accountsNFTInfo[account][nftLevel].amount;
        uint256 earning = value.mul(timeLong).mul(amount);
        return earning;
    }

    function updateAccountNFTEaring(address account, uint256 nftLevel)
        internal
    {
    ...
    function lpMiningNumbersPerSecond(uint256 amount, uint256 totalAmount)
        internal
        pure
        returns (uint256)
    {
    ...
    function getAccountLPEarning(address account)
        internal
        view
        returns (uint256)
    {
    ...
    function isNFTEmpty(address account) internal view returns (bool) {
        return
            (accountsNFTInfo[account][AUTH1].stakeTime == 0) &&
    ...
    function stakeNFT(uint256 nftLevel, uint256 amount) external {
        require(
            nftLevel >= AUTH1 && nftLevel <= AUTH6,
            "NFT Level is invalid!"
        );
        ...
        accountsNFTInfo[msg.sender][nftLevel].amount = accountsNFTInfo[
            msg.sender
        ][nftLevel]
            .amount
            .add(amount);
    ...
    function unStakeNFT(uint256 nftLevel, uint256 amount) external {
        ...
        require(
            amount > 0 &&
                amount <= accountsNFTInfo[msg.sender][nftLevel].amount,
            "Amount should be bigger than 0 and not bigger than owned!"
        );
    ...
    function stakeLPToken(uint256 amount) external {
        ...
        accountsLPInfo[msg.sender].stakeTime = now;
        accountsLPInfo[msg.sender].amount = accountsLPInfo[msg.sender]
            .amount
            .add(amount);
    ...
    function unStakeLPToken() external {
        require(
            ...
            "This account has not staked LP Token yet!"
        );
    ...
    function claimEarning() external {
        ...
        if (
            !isNFTEmpty(msg.sender) && accountsLPInfo[msg.sender].stakeTime != 0
        ) {
            for (uint256 i = AUTH1; i <= AUTH6; i++) {
                updateAccountNFTEaring(msg.sender, i);
            ...
            for (uint256 i = AUTH1; i <= AUTH6; i++) {
                if (accountsNFTInfo[msg.sender][i].stakeTime != 0) {
                    ...
                    totalEarning = totalEarning.add(earning);
                    accountsNFTInfo[msg.sender][i].earning = 0;
                    accountsNFTInfo[msg.sender][i].stakeTime = 0;
                }
            }
    ...
    function getEarning() external view returns (uint256) {
        ...
        if (
            ...
            for (uint256 i = AUTH1; i <= AUTH6; i++) {
                ...
                totalEarning = totalEarning.add(nftOldEarning).add(
                    nftNewEarning
                );
            }
        }
