...
library QueueLib {
    ...
    function get(Queue storage queue, uint256 index)
        internal
        view
        returns (uint256)
    {
        require(
            queue._read + index < queue._array.length,
            "get: Out of index."
        );
...
contract AuthmenNFT is ERC1155BurnableUpgradeable {
    ...
    function initialize(address authmen, address trytoken) public initializer {
        ...
        require(trytoken != address(0), "try address required");
        _authmen = authmen;
        _trytoken = trytoken;
    }
    ...
    function stakeTRY() external {
        require(
            (stakeTime[msg.sender].length() == 0) ||
                (stakeTime[msg.sender].length() != 0 &&
                    now.sub(
                        stakeTime[msg.sender].get(
                            stakeTime[msg.sender].length() - 1
                        )
                    ) >
                    TRY_LOCK_TIME),
            "TRY is locking"
    ...
    function latestTRYStakeTime() external view returns (uint256) {
        if (stakeTime[msg.sender].length() == 0) {
            ...
            return
                stakeTime[msg.sender].get(stakeTime[msg.sender].length() - 1);
        }
    }
}